...
from local.constants import FIRST_QUESTION, TERMINATE
...
class Chat(object):
    def __init__(self, pairs=(), reflections=None, call=_function_call,
                 api=None, normalizer=None, default_template=None, local_path=None):
        """
        ...
        self.substitution = Substitution(self.local_path)
        ...
        if default_template is None:
            default_template = path.join(self.local_path, "dictions.template")
    ...
    def __response_on_topic(self, session, text, previous_text, current_topic):
        match = self.__intend_selection(text, previous_text, current_topic)
        if match:
    ...
    def _respond(self, session, text):
        ...
        while current_topic_order:
            try:
                return self.__response_on_topic(session, text, previous_text, current_topic)
            except ValueError:
                pass
    ...
    def converse(self, first_question=None, terminate="end", gui=None, session_id="general"):
        """
...
def app(first_question=None, gui=None, **kwargs):
    if not first_question:
        first_question = FIRST_QUESTION["intro"]
    terminate = TERMINATE["quit"]
    Chat(**kwargs).converse(first_question, terminate=terminate, gui=gui)
